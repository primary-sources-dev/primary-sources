...
async function loadBlogPost(slug) {
    try {
        const response = await fetch(`${window.basePath || ''}assets/data/blog.json`);
...
function populateRelatedPosts(relatedSlugs, allPosts) {
    ...
    grid.innerHTML = relatedPosts.map(post => `
        <a href="${window.basePath || ''}pages/blog-post.html?slug=${post.slug}" class="block border border-archive-secondary/20 bg-[#252021]/60 hover:border-primary transition-colors overflow-hidden group">
            <div class="p-6">
...
function showErrorState(message) {
    ...
    container.innerHTML = `
        <div class="text-center py-20">
            ...
            <a href="${window.basePath || ''}pages/blog.html" class="inline-flex items-center gap-2 text-xs text-primary hover:underline uppercase tracking-widest">
